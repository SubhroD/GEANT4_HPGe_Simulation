...
G4double norm_dist(double sg)
{
  unsigned seed = chrono::system_clock::now().time_since_epoch().count();
  default_random_engine generator(seed);
  normal_distribution<double> distribution(0.0, sg);
  G4double n = distribution(generator) * CLHEP::keV;
  return n;
}
...
void B4aEventAction::EndOfEventAction(const G4Event *event)
{
  ...
  if (fEnergyAbs <= 0.67 * MeV && fEnergyAbs > 0.654 * MeV)
  {
    fEnergyAbs = fEnergyAbs + norm_dist(0.6375);
  }
